...
/**
 * @brief Construct a new Bmi 088 Accel:: Bmi 088 Accel object
 *
 * @param bus SPI bus
...
/**
 * @brief begins communication with the BMI088
 *
 * @return int error
 */
int Bmi088Accel::begin()
{
  ...
  if (!isCorrectId())
  {
    Serial.println(F("BMI088: ID Registry Failed"));
    return -1;
  }
  /* soft reset */
  softReset();
  ...
  if (!setPower(true))
  {
    Serial.println(F("BMI088: Power Set Failed"));
    return -2;
  }
  /* enter active mode */
  if (!setMode(true))
  {
    Serial.println(F("BMI088: Mode Set Failed"));
    return -3;
  }
  /* self test */
  if (!selfTest())
  {
    Serial.println(F("BMI088: Self Test Failed"));
    return -4;
  }
  /* soft reset */
  softReset();
  ...
  {
    Serial.println(F("BMI088: Range Set Failed"));
    return -7;
  }
  /* set default ODR */
  if (!setOdr(ODR_1600HZ_BW_280HZ))
  {
    Serial.println(F("BMI088: ODR Failed"));
    return -8;
  }
  /* check config errors */
  if (isConfigErr())
  {
    Serial.println(F("BMI088: Config Error Detected"));
    return -9;
  }
  /* check fatal errors */
  if (isFatalErr())
  {
    Serial.println(F("BMI088: Fatal Error Detected"));
    return -10;
  }
  return 1;
}
...
void Bmi088Accel::readSensor()
{
  ...
  accel_mss[0] = (double)(accel[0] * tX[0] + accel[1] * tX[1] + accel[2] * tX[2]) / 32768.0 * accel_range_mss;
  accel_mss[1] = (double)(accel[0] * tY[0] + accel[1] * tY[1] + accel[2] * tY[2]) / 32768.0 * accel_range_mss;
  accel_mss[2] = (double)(accel[0] * tZ[0] + accel[1] * tZ[1] + accel[2] * tZ[2]) / 32768.0 * accel_range_mss;
  /* time data */
  ...
  temp_c = (double)temp_int11 * 0.125 + 23.0;
}

/* returns the x acceleration, m/s/s */
double Bmi088Accel::getAccelX_mss()
{
  return accel_mss[0] - x_bias;
}

/* returns the y acceleration, m/s/s */
double Bmi088Accel::getAccelY_mss()
{
  return accel_mss[1] - y_bias;
}

/* returns the z acceleration, m/s/s */
double Bmi088Accel::getAccelZ_mss()
{
  return accel_mss[2] - z_bias;
}

/* returns the temperature, C */
double Bmi088Accel::getTemperature_C()
{
  return temp_c;
}
...
void Bmi088Accel::estimateBias(uint8_t quantity)
{
  double xIntegration = 0;
  double yIntegration = 0;
  double zIntegration = 0;

  for (uint8_t i = 0; i < quantity + 4; i++)
  {
    readSensor();
    if (i >= 4) // first four readings are incorrect so we omit them
    {
      xIntegration += accel_mss[0];
      yIntegration += accel_mss[1];
      zIntegration += accel_mss[2];
    }
  }

  x_bias = (xIntegration / quantity) - G;
  y_bias = yIntegration / quantity;
  z_bias = zIntegration / quantity;
}
...
{
  uint8_t writeReg = 0;
  double accel_pos_mg[3], accel_neg_mg[3];
  /* set 24G range */
  setRange(RANGE_24G);
...
void Bmi088Gyro::readSensor()
{
  ...
  gyro_rads[0] = (double)(gyro[0] * tX[0] + gyro[1] * tX[1] + gyro[2] * tX[2]) / 32767.0 * gyro_range_rads;
  gyro_rads[1] = (double)(gyro[0] * tY[0] + gyro[1] * tY[1] + gyro[2] * tY[2]) / 32767.0 * gyro_range_rads;
  gyro_rads[2] = (double)(gyro[0] * tZ[0] + gyro[1] * tZ[1] + gyro[2] * tZ[2]) / 32767.0 * gyro_range_rads;
}

/* returns the x gyro, rad/s */
double Bmi088Gyro::getGyroX_rads()
{
  return gyro_rads[0];
}

/* returns the y gyro, rad/s */
double Bmi088Gyro::getGyroY_rads()
{
  return gyro_rads[1];
}

/* returns the z gyro, rad/s */
double Bmi088Gyro::getGyroZ_rads()
{
  return gyro_rads[2];
}
...
double Bmi088::getAccelX_mss()
{
  return accel->getAccelX_mss();
}

double Bmi088::getAccelY_mss()
{
  return accel->getAccelY_mss();
}

double Bmi088::getAccelZ_mss()
{
  return accel->getAccelZ_mss();
}

double Bmi088::getTemperature_C()
{
...
double Bmi088::getGyroX_rads()
{
  return gyro->getGyroX_rads();
}

double Bmi088::getGyroY_rads()
{
  return gyro->getGyroY_rads();
}

double Bmi088::getGyroZ_rads()
{
